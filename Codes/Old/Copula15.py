...
def recur(panel,u,tol,pnmc):
    ...
    if d0 <= tol or d1 <= tol:
        ...
        nn = 5
        xGrid = np.linspace(x0,x1,nn,endpoint=True)
        e = outer(xGrid,ones(len(pnmc)))-outer(fmu(pnmc),ones(nn)).T
        FxGrid = norm.cdf(e,scale=sdx).mean(axis=1)
        panelNewValues= np.stack((xGrid,FxGrid),axis=1)
        x,panel1 = recur(panelNewValues,u,tol,pnmc)
        if i == 0:
            panel2 = np.vstack((panel1[:-1],panel))
        elif i == n:
            panel2 = np.vstack((panel,panel1[1:])) 
        else:            
            panel2 = np.insert(panel,i,panel1[1:-1],axis=0)  
        return x,panel2
...
n = 10000
#%%
def buildRev(fxmc,Fx,pnmc,tol,n,Fz):
    ...
    for i in range(n):
        #i = np.searchsorted(panel[:,1],u)
        x, panel = recur(panel,U1[i],tol,pnmc0)
#        x1 = newton(getFx,x0=panel[idxU[i],0],fprime=getfx,
#                    args=(mu,U1[i]))
#        x2 = bisect(getFx,a=panel[idxU[i]-1,0],b=panel[idxU[i],0],
#                    args=(mu,U1[i]))
        X[i] = x
        #print(i)
    t1 = time.time()
