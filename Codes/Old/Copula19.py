...
def buildRev3(xGrid,fmupnmc,Fx,Fz):
    ...
    x = norm.ppf(U[0],loc=fmupnmc,scale=sdx).mean()
    x = xGrid.min()    
...
graphDensity2(t+1,nff,stack([kalman,x1density]).T,pnmc)
#%%
for t in range(1,T-1):
    ...
    Fx = rvx.cdf(tile(xGrid,(N,1))-tile(fmupnmc,(ndx,1)).T).mean(0)
    Fz = rvz.cdf(Z[t]-pnmc).mean()
    pnmc = buildRev3(xGrid,fmupnmc,Fx,Fz)
    graphDensity2(t+1,nff,stack([kalman,x1density]).T,pnmc)